...
def _describe_etf_sync(result: dict) -> str:
    ...
    if result.get("error") == "already current":
        return (
            f"ETF 已覆盖 {result.get('target_date', '最近完整交易日')} "
            f"({result.get('coverage_count', 0)}/{result.get('symbol_count', 0)}只)，无需扫描"
        )
    if not result.get("is_trade_day", True):
        return "ETF 已跳过（非交易日）"
    return "ETF 已跳过"
...
def main() -> None:
    ...
    try:
        ...
        if args.backfill:
            ...
            r2 = sync_mgr.sync_etf_daily(force=True, full_history=True)
