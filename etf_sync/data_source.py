    ...
    def get_daily(self, code: str, days: int = 5) -> Optional[pd.DataFrame]:
        ...
        if self.active_source == "tencent":
            ...
            if df is not None and not df.empty:
                ...
                # 腾讯返回的是前复权序列。即使其历史长度短于请求值，也不能
                # 为了“更长”而用未复权的 Sina 整段替换，否则会污染收益和回测。
                df.attrs.update(source=self.last_source, adjustment="qfq" if self.last_source == "tencent" else "none")
                return df
            # 腾讯失败 → 切到 Sina
            ...
            if df is not None and not df.empty:
                ...
                df.attrs.update(source="sina", adjustment="none")
                return df
            return None
