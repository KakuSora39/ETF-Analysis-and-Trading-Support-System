...
class PositionState:
    ...
    entry_strategies: list[str] = field(default_factory=list)
    entry_reason: str = ""
...
class SimState:
    """完整模拟盘状态。"""
    version: int = 5               # 版本5 增加买入理由和机会类型字段
...
class StateManager:
    ...
    def _from_dict(self, raw: dict) -> SimState:
        ...
        return SimState(
            version=max(raw.get("version", 1), 5),
            ...
            position=PositionState(
                ...
                name=pos.get("name", ""),
                opportunity_type=pos.get("opportunity_type", ""),
                entry_market_regime=pos.get("entry_market_regime", ""),
                entry_strategies=pos.get("entry_strategies", []),
